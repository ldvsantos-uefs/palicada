...
## 2.9 Estimativa da degradação estrutural

A vida útil funcional das paliçadas não depende apenas da capacidade de retenção, mas também da integridade mecânica do material construtivo. A taxa de degradação da resistência à tração e flexão do *Bambusa vulgaris* foi modelada com base em curvas de decaimento biológico reportadas na literatura para bioengenharia em ambiente tropical úmido [@ghimire_et_al_2013], ajustadas para um cenário de contato direto com solo úmido. Definiu-se uma função de decaimento logístico com ponto crítico de falha estrutural (50% de perda de resistência) em 5 anos, compatível com a durabilidade média observada em intervenções similares sem tratamento químico preservativo. Essa curva de degradação foi contraposta às curvas de preenchimento de capacidade para identificar o ponto de interseção onde a falha por colapso material precede ou sucede a falha funcional por assoreamento completo.
...
Para sintetizar a variabilidade hidrológica no período monitorado, as precipitações mensais foram organizadas em classes por quantis; a Tabela 4 resume os limiares operacionais, o tamanho amostral e a sedimentação média mensal por segmento.
...
![Figura 11 -- (a) Enquadramento regional (IDF) e (b) leitura temporal da precipitação no período monitorado, com identificação de eventos extremos (P95).](media/artigo/21a_curvas_idf_serie_temporal.png){width="6.5in"}
...
![Figura 12 -- (a) Recorte espacial da Ravina 1 utilizado como domínio de referência para as simulações, com a representação do canal e a localização das paliçadas; (b) simulação espacial para o limiar P90; (c) simulação espacial para o limiar P95. ](media/artigo/fig_ravina1_mapas_sim_p90_p95_recorte_tif_one_event.png){width="6.5in"}
...
A simulação espacial do preenchimento sedimentar (Figura 13) ilustra a evolução temporal da acumulação a montante das paliçadas em quatro estágios: 25%, 50%, 75% e 100% da capacidade máxima. As curvas de nível evidenciam a formação de cunhas deposicionais com decaimento longitudinal, características de barreiras permeáveis transversais ao fluxo [@xu_fu_he_2013].

A distribuição espacial mostra preenchimento preferencial nas zonas imediatamente a montante de cada estrutura, compatível com a redução da tensão cisalhante e o aumento do tempo de residência hidráulica nessa região [@wang_et_al_2021]. A Figura 13a representa o estágio atual do sistema (<2% de ocupação). À medida que o preenchimento avança (Figuras 13b, 13c e 13d), observa-se a progressão da cunha deposicional até a crista, com redução do volume remanescente para acomodar novos aportes e aumento da probabilidade de extravasamento ou *bypass*.


![Figura 13 -- Simulação espacial da evolução temporal do preenchimento sedimentar nas paliçadas: (a) 25% da capacidade (~0,2-1,2 anos), (b) 50% (~0,4-2,4 anos), (c) 75% (~0,6-3,6 anos), e (d) saturação (100%, 0,8-4,8 anos).  ](media/artigo/fig_espacial_temporal_capacidade.png){width="6.5in"}
...
O contraste entre a vida útil material do bambu (~4-5 anos) e as estimativas de preenchimento revela dinâmicas complementares. Enquanto a taxa média deposicional projeta uma longevidade hidráulica estendida em cenário de base, a simulação sob eventos extremos (P90 e P95) delimita o horizonte operacional crítico entre 0,8 e 4,8 anos. Este intervalo apresenta forte aderência à janela de integridade estrutural do bambu, sugerindo que a saturação física e o limite de resistência mecânica convergem temporalmente. Dessa forma, o dimensionamento valida-se pela capacidade do sistema em acomodar picos de carga sólida dentro do ciclo de vida útil da bioengenharia, estabelecendo a recorrência de extremos como o parâmetro diretor para os protocolos de manutenção e desassoreamento.

A menor longevidade projetada para o segmento inferior reflete simultaneamente a menor altura útil disponível (36 cm vs. 50 e 76 cm) e a maior eficiência de retenção ($1,97 \times 10^{-4}$ cm/mm), o que acelera o preenchimento relativo. Esse resultado sugere que a manutenção preventiva deve priorizar o segmento inferior, onde a capacidade remanescente será esgotada em escala temporal mais curta. Sob cenários de precipitação extrema (P95), o horizonte estimado de aproximadamente 0,8 a 1,0 ano é, de fato, o limitante operacional crítico, sobrepondo-se à preocupação com a biodegradação. Portanto, diferentemente de sistemas de baixa taxa de deposição, a saturação física por preenchimento completo representa a limitação imediata; o fator restritivo primário é a gestão do volume retido (limpeza) para evitar o soterramento funcional e o extravasamento não controlado.


Quanto à interação entre degradação estrutural e capacidade de retenção, a sustentabilidade da intervenção a longo prazo depende do balanço entre a taxa de assoreamento (falha funcional) e a taxa de decomposição do material orgânico (falha estrutural). A literatura indica que o desempenho mecânico de estruturas de bioengenharia decai exponencialmente, enquanto a função biológica assume protagonismo gradual [@romano_et_al_2016]. A Figura 15 materializa essa transição ao contrapor o registro de campo com a modelagem de desempenho. O estabelecimento da barreira viva (Figura 15a) evidencia o vigor vegetativo inicial das estacas, sinalizando o potencial para sucessão funcional. Em paralelo, as curvas projetadas (Figura 15b) indicam que, sob cenários de alta energia (P90 e P95), a capacidade de retenção atinge a saturação (>100%) em menos de 2 anos, muito antes do declínio crítico da resistência do bambu, previsto para ocorrer após o 4º ano, padrão consistente com estudos de durabilidade de madeira em contato com solo [@huzita_noda_kayo_2020].

![Figura 15 -- (a) Estabelecimento da Barreira Viva; (b) Interação entre degradação da resistência estrutural (Lit.) e evolução da capacidade e vegetação (Sim.).](media/artigo/Fig_15_painel_integrado.png){width="6.5in"}

Isso indica que, em regime de eventos extremos, o principal modo de falha é o transbordamento por assoreamento, exigindo desassoreamento frequente antes da falha estrutural. Por outro lado, o Fator de Vegetação apresenta crescimento logístico que cruza a curva de degradação estrutural por volta do 3º ano, sugerindo que o sistema transita de uma barreira puramente mecânica para uma estabilização biotécnica, onde o enraizamento e a cobertura vegetal assumem progressivamente a função de contenção à medida que a estrutura inerte se decompõe. Esse processo é documentado por @bombino_et_al_2019 e @holanda_et_al_2021, que destacam a importância da colonização vegetal para a estabilidade de longo prazo após o colapso físico do bambu.
...
Em termos operacionais, o sistema demanda manutenção preventiva e gestão do risco de extravasamento, sobretudo após episódios de maior energia que aceleram o preenchimento do armazenamento e ampliam a probabilidade de passagem preferencial de sedimentos. O dimensionamento e a operação devem contemplar extravasamento controlado e integração com a estabilização de taludes, assegurando a transição para uma estrutura viva. Nesse regime consolidado, a estabilidade passa a ser governada pela ancoragem por rizomas e pela dissipação hidráulica promovida pela densidade de colmos, garantindo a funcionalidade do sistema e o controle do escoamento concentrado além da vida útil do material construtivo.
